...
defmodule Solution do
  ...
  def add_two_numbers(l1, l2) do
    with  reverse_l1 <- reverse(l1, nil),
          reverse_l2 <- reverse(l2, nil),
          added <- add(reverse_l1, reverse_l2, 0)
    do
      reverse(added, nil)
    end
  end
...
defmodule Tests do
  ...
  def example4 do
    list1 = %ListNode{val: 7, next: %ListNode{val: 2, next: %ListNode{val: 4, next: %ListNode{val: 3}}}}
    list2 = %ListNode{val: 5, next: %ListNode{val: 6, next: %ListNode{val: 4}}}
    IO.puts(list_to_string(Solution.add_two_numbers(list1, list2)))
  end
end
